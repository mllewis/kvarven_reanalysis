...
TIDY_DATA_PATH <- here("data/tidy_krvaven.csv")
...
ggplot(wide_df, aes(x = es_TF_MA,
                    y = es_replication_studies)) +
  geom_point(size = 4) +
...
lm(es_replication_studies ~ log(es_TF_MA) , data = wide_df) %>%
  summary()
...
wide_df <- tidy_data %>%
  ...
  left_join(tidy_data %>% filter(table_name == "TF_MA") %>%
              select(id, ci_lower, ci_upper)) %>%
